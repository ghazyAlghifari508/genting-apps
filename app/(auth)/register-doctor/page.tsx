...
import { registerDoctorStandalone } from '@/services/doctorRegistrationService'
...
export default function RegisterDoctorPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.specialization || !formData.licenseNumber || !formData.hourlyRate) {
      toast({ title: 'Mohon lengkapi data profesional', variant: 'destructive' })
      return
    }
    ...
    setLoading(true)
    try {
      await registerDoctorStandalone({
        ...formData,
        email: cleanEmail,
      ...
      toast({ 
        ...
        description: 'Mengarahkan ke halaman status...' 
      })

      // Redirect to the new pending status page
      ...
      <div className="w-full max-w-2xl bg-white  rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.08)]  border border-slate-100  overflow-hidden transition-all">
        ...
        <div className="px-8 sm:px-12 py-8 bg-white  transition-colors">
          <AnimatePresence mode="wait">
            ...
            >
              {step === 1 && <AccountInfoStep formData={formData} setFormData={setFormData} />}
              {step === 2 && <PersonalInfoStep formData={formData} setFormData={setFormData} />}
              {step === 3 && <ProfessionalInfoStep formData={formData} setFormData={setFormData} />}
            </motion.div>
          </AnimatePresence>
        </div>
        ...
        <div className="px-6 sm:px-8 py-4 bg-slate-50  border-t border-slate-100  flex flex-col sm:flex-row justify-between items-center gap-3 transition-colors">
          <div>
            <p className="text-[10px] font-black text-slate-400  uppercase tracking-[0.2em]">Step {step} of 3</p>
          </div>
