...
export default function BlogSection() {
  ...
  return (
    <section className="py-24 bg-white  transition-colors" id="blog">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-slate-900  leading-tight mb-4 transition-colors">
            ...
            {posts.map((post, idx) => (
              <div key={idx} className="flex gap-5 items-center group cursor-pointer">
                <div className="flex-1">
                  <div className="flex items-center gap-4 text-xs text-slate-500  mb-2 transition-colors">
                    <span className="px-3 py-1 rounded-full bg-doccure-gray  text-doccure-teal font-bold transition-colors">
                      {post.category}
                    </span>
                    <span>{post.date}</span>
                  </div>
                  <h3 className="text-base md:text-lg font-bold text-slate-900  leading-snug group-hover:text-doccure-teal transition-colors">
                    {post.title}
                  </h3>
                </div>
                <Image
                  ...
                  width={112}
                  height={80}
                  className="w-28 h-20 rounded-2xl object-cover flex-shrink-0 grayscale-[0.2] group-hover:grayscale-0 transition-all shadow-md  border border-transparent "
                />
              </div>
            ))}
          </div>
